...
class MinMaxStats(object):
    ...
    BMO: MuZero computes normalized Q value estimates Q ∈[0,1] by using the
    minimum-maximum values observed in the search tree up to that point. When
    a node is reached during the selection stage, the algorithm computes the
    normalized Qvalues of its edges to be used in the pUCT rule
    """
...
class MuZeroConfig(object):
    ...
    def __init__(
        self,
        action_space_size: int,  # BMO: action space enumerates a superset of all possible actions
        max_moves: int,  # BMO: 27k for Atari
        discount: float,  # BMO: discounted intermediate rewards
        dirichlet_alpha: float,  # BMO : dirichlet noise to encourage the search to explore new actions.
        num_simulations: int,  # BMO: 50 simulations for each search in Atari
        batch_size: int,  # BMO 1024 for Atari
        td_steps: int,
        num_actors: int,
        lr_init: float,
        lr_decay_steps: float,
        visit_softmax_temperature_fn,  # BMO: lower over time, action selection becomes greedier as training progresses,
...
def play_game(config: MuZeroConfig, network: Network) -> Game:
    ...
    while not game.terminal() and len(game.history) < config.max_moves:
        ...
        backpropagate(
            [root], network_output.value, game.to_play(), config.discount, min_max_stats
        )
